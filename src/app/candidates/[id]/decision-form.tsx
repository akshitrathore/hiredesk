...
function SubmitButton({
  ...
  const className =
    variant === "danger"
      ? "h-10 rounded-md bg-red-700 px-4 text-sm font-semibold text-white transition hover:bg-red-800 disabled:cursor-not-allowed disabled:opacity-60"
      : "h-10 rounded-md bg-accent px-4 text-sm font-semibold text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-60";
...
export function CandidateDecisionPanel({
  ...
  return (
    <section className="rounded-lg border border-line bg-panel p-5">
      ...
      {isTerminal ? (
        <p className="mt-4 rounded-md border border-line bg-background px-3 py-2 text-sm text-muted">
      ...
      <div className="mt-4 grid gap-4">
        ...
        <form action={rejectedAction} className="space-y-3">
          ...
          <label className="block">
            ...
            <textarea
              className="mt-2 min-h-20 w-full rounded-md border border-line bg-white px-3 py-2 text-sm outline-none focus:border-foreground disabled:bg-background disabled:text-muted"
              disabled={isTerminal}
              name="reason"
